package FirstStap.algoritms.common.bfs;
...
public class BFSAdjacencyListIterative {
    ...
    private void bfs(int start) {
        ...
        // Start by visiting the 'search' node and add it to the queue
        queue.offer(start);
        visited[start] = true;
    ...
    // Initialize an empty adjacency list that can hold up to n node.
    public static List<List<Edge>> createAnEmptyGraph(int n) {
        List<List<Edge>> graph = new ArrayList<>(n);
        for (int i = 0; i < n; i++) graph.add(new ArrayList<>());
        return graph;
    }

    // Add a directed edge form node 'u' to node 'v' with cost 'cost'
    ...
    // will have a weight of 1 since it's intended to be unweighted.
